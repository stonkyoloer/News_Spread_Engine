...
```python
# OUTPUT EXPECTED [python markdown for copy & paste into sectors.py]:
...
- Credit rating actions from Moody’s/S&P/Fitch (public releases).
...
- Size & Price: Market cap ≥ $1B and last close ≥ $20 as of the research date (with source/date).
...
- If specific required information cannot be confirmed, state “Information not available.”
...
- Include a mix of large-cap (>$50B) and mid-cap ($5B–$50B) names (cite size source/date).
...
- Prefer names with recent (≤90 days) public news in ratings/index/analyst-headline categories.
...
## ◽️ Output | Defines the Trading Portfolio
...
## ◽️ Output | Defines Trade Type, DTE, Entry and Exit Plans
